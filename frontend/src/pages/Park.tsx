...
export function Park() {
  ...
  const submitParkListing = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm({ name: '', locality: '', address: '', features: '', hours: '', phone: '', email: '', website: '' });
  ...
  if (
    !hasCategory(city, 'park') ||
    (!parksQuery.isLoading && !parksQuery.isError && allSpots.length === 0)
  ) {
    return (
      <ComingSoon
        emoji="🌳"
